...
1. **메인 로비 (Home Screen)**: 블랙잭과 룰렛 게임의 간단한 규칙과 바로가기 버튼을 제공합니다.
2. **공통 포인트 관리**: 사이드바에 보유 포인트를 표시하고, 포인트가 떨어지면 언제든 무료 충전이 가능하도록 설계되었습니다.
3. **룰렛 (Roulette) 구현**:
   - **베팅 옵션**: 색상(Red/Black, 2배), 홀짝(Odd/Even, 2배), 특정 숫자(0~36, 36배)를 선택할 수 있습니다.
   - **돌리기 애니메이션**: 숫자가 돌아가는 연출 효과와 정산 판정 기능이 내장되어 있습니다.
